(() => {
  ...
  async function getProfile() {
    ...
    const rows = await jsonRequest(
      `/rest/v1/profiles?user_id=eq.${encodeURIComponent(uid)}&select=role,device_name,active`
    );
  ...
  // Admin registration reads now use secure RPC functions.
  ...
  async function getRegistrationsAfter(id) {
    return rpc(
      "get_admin_registrations_after",
      {
        p_after_id: Number(id) || 0
      }
    );
  }
  ...
  async function getSystemState() {
    const rows = await jsonRequest(
      "/rest/v1/system_state?id=eq.1&select=reset_version,updated_at"
    );
